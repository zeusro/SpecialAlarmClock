...
import com.gdtel.eshore.androidframework.ui.slidingmenu.lib.SlidingMenu.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
	...
	public void scrollBehindTo(View content, int x, int y,boolean isScrolled) {
		int vis = View.VISIBLE;	
		if(isScrolled){
			vis = contentScroll(content, x, y, vis);
		}else{
			vis = contentNotScroll(content, x, y, vis);
		}
	...
	private int contentScroll(View content, int x, int y, int vis) {
	...
	private int contentNotScroll(View content, int x, int y, int vis) {
		if (mMode == SlidingMenu.LEFT) {
			if (x >= content.getLeft()) vis = View.INVISIBLE;
			scrollTo((int)((x + getBehindWidth())*1), y);
		} else if (mMode == SlidingMenu.RIGHT) {
			if (x <= content.getLeft()) vis = View.INVISIBLE;
			scrollTo((int)(getBehindWidth() - getWidth() + 
					(x-getBehindWidth())*1), y);
		} else if (mMode == SlidingMenu.LEFT_RIGHT) {
			mContent.setVisibility(x >= content.getLeft() ? View.INVISIBLE : View.VISIBLE);
			mSecondaryContent.setVisibility(x <= content.getLeft() ? View.INVISIBLE : View.VISIBLE);
			vis = x == 0 ? View.INVISIBLE : View.VISIBLE;
